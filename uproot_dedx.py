...
branch_list = [
    ...
    b"Tracks.fTPCsignalS", # TPC dE/dx RMS
    b"Tracks.fTPCncls" # Number of TPC clusters
]
...
def run_analysis():
    ...
    plt.scatter(totalMomenta,trackdEdx,s=1)
    plt.show()

def getBeta(entry):
    t = entry[b"Tracks.fTrackTime[5]"]
    d = entry[b"Tracks.fTrackLength"]
    nhits = entry[b"Tracks.fTPCncls"]
    betas = []
    for i_evt in range(len(d)):
        for i_track in range(len(d[i_evt,:])):
            if d[i_evt,i_track] > 0:
                if t[i_evt,i_track,4] > 0:
                    if nhits[i_evt,i_track] > 50:
                        b = ((d[i_evt,i_track]/100.) / (t[i_evt,i_track,4]/(10**12))) / (3*10**8)
                        betas.append(b)
    return np.array(betas)

def getdEdx(entry):
    dEdx = entry[b"Tracks.fTPCsignalS"]
    nhits = entry[b"Tracks.fTPCncls"]
    eloss = []
    l = entry[b"Tracks.fTrackLength"]
    t = entry[b"Tracks.fTrackTime[5]"]
    for i_evt in range(len(dEdx)):
        for i_track in range(len(dEdx[i_evt,:])):
            if l[i_evt,i_track]>0 and t[i_evt,i_track,4]>0 and nhits[i_evt,i_track]>50:
                eloss.append(dEdx[i_evt,i_track])
    return np.array(eloss)
...
def mapGetMomentum(entry):
    ...
    for i_evt in range(len(alphas)):
        for i_track in range(len(alphas[i_evt,:])):
            if d[i_evt,i_track] > 0 and t[i_evt,i_track,4] > 0 and nhits[i_evt,i_track]>50:
